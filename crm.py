...
st.set_page_config(
    page_title=" Bework | CRM",
    page_icon="💼",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': 'https://bework.com/soporte',
        'Report a bug': 'https://beworks.com/contacto',
        'About': '**Bework CRM** - Plataforma integral de gestión de clientes y relaciones comerciales'
    }
)
...
# (Se eliminó el manejo temprano de OAuth para usar el flujo estándar más abajo)
...
if not st.session_state.drive_creds:
    ...
    try:
        qp = st.query_params
        if "code" in qp:
            st.sidebar.info("Procesando autorización de Google…")
            try:
                code_auto = qp["code"]
                if isinstance(code_auto, (list, tuple)):
                    code_auto = code_auto[0] if code_auto else ""
                client_config = {
                    "web": {
                        "client_id": CLIENT_ID,
                ...
                flow.fetch_token(code=code_auto)
                st.session_state.drive_creds = flow.credentials
                st.session_state.processed_auth_code = code_auto
                try:
    ...
    # Botón de prueba rápida de subida a Drive
    if st.sidebar.button("🧪 Probar subida a Drive", help="Sube un archivo de prueba a la carpeta raíz 'CRM Kapitaliza'"):
        try:
            def _drive_smoke_test() -> str | None:
                if not st.session_state.drive_creds:
                    return None
                svc = build("drive", "v3", credentials=st.session_state.drive_creds)
                # Asegurar carpeta raíz 'CRM Kapitaliza'
                query_root = "name='CRM Kapitaliza' and mimeType='application/vnd.google-apps.folder' and trashed=false"
                results_root = svc.files().list(q=query_root, fields="files(id, name)").execute()
                if not results_root.get('files'):
                    meta_root = {'name': 'CRM Kapitaliza', 'mimeType': 'application/vnd.google-apps.folder'}
                    root = svc.files().create(body=meta_root, fields='id').execute()
                    root_id = root.get('id')
                else:
                    root_id = results_root['files'][0]['id']
                # Asegurar carpeta de prueba
                test_folder_name = "Cliente_TEST_SMOKE"
                query_test = f"name='{test_folder_name}' and mimeType='application/vnd.google-apps.folder' and '{root_id}' in parents and trashed=false"
                results_test = svc.files().list(q=query_test, fields="files(id, name)").execute()
                if not results_test.get('files'):
                    meta_test = {'name': test_folder_name, 'mimeType': 'application/vnd.google-apps.folder', 'parents': [root_id]}
                    test_folder = svc.files().create(body=meta_test, fields='id').execute()
                    test_folder_id = test_folder.get('id')
                else:
                    test_folder_id = results_test['files'][0]['id']
                # Subir archivo simple
                import io, time
                fname = f"smoke_test_{int(time.time())}.txt"
                file_meta = {"name": fname, "parents": [test_folder_id]}
                media = MediaIoBaseUpload(io.BytesIO(b"OK"), mimetype="text/plain")
                file = svc.files().create(body=file_meta, media_body=media, fields="id, webViewLink").execute()
                # Hacer público
                try:
                    svc.permissions().create(fileId=file.get('id'), body={'type': 'anyone', 'role': 'reader'}).execute()
                except Exception:
                    pass
                return file.get("webViewLink")

            link = _drive_smoke_test()
            if link:
                st.sidebar.success(f"✅ Subida de prueba OK: {link}")
            else:
                st.sidebar.warning("No fue posible completar la subida de prueba.")
        except Exception as e:
            st.sidebar.error(f"Error en prueba de Drive: {e}")
...
GSHEET_ESTATUSINBURSA_TAB = "estatusinbursa"
...
def crear_carpeta_cliente_drive(cliente_id, cliente_nombre=""):
    """Crea o encuentra la carpeta del cliente en Google Drive (raíz: 'CRM Kapitaliza')."""
    if not st.session_state.drive_creds:
        return None
        ...
        if ids_quick:
            ...
            if cid_quick:
                ...
                with st.form(f"form_edit_cliente_{cid_quick}", clear_on_submit=False):
                    ...
                    # Fuente y tipo_tramite minimal (aparecen fuera para lógica previa)
                    ...
                    fuente_select = st.selectbox("Fuente", ["LEADS","LUZWARE", "SEGUIMIENTO"], index=( ["LEADS","LUZWARE","SEGUIMIENTO"].index(cur.get("fuente")) if cur.get("fuente") in ["LEADS","LUZWARE","SEGUIMIENTO"] else 0), key=f"edit_fuente_select_{cid_quick}")
                    ...
                    if st.form_submit_button("Guardar cambios cliente"):
                        ...
                        if cid_quick not in base.index:
                            ...
                            try:
                                ...
                                diffs = []
                                editable_cols = [
                                    "nombre","sucursal","telefono","fecha_ingreso","estatus","capacidad",
                                    "monto_propuesta","plazo","estado_civil","tipo_vivienda","correo",
                                    "ref1_nombre","ref1_telefono","ref1_parentesco","ref2_nombre","ref2_telefono",
                                    "ref2_parentesco","antiguedad_cuenta","usuario_cipre","contrasena","tipo_tramite","fuente","asesor"
                                ]
...
with tab_import:
    ...
    if up_excel:
        ...
        if df_imp_raw.empty:
            ...
            # Si el archivo trae columna `producto`, usar catálogos de estatus por producto
            def get_estatus_catalog_for_product(prod: str) -> list:
                """Devuelve la lista de estatus correspondiente al producto (Inbursa/Multiva/Mejoravit) o la lista global por defecto."""
            ...
            if "producto" in df_norm.columns:
                # aplicar canonicalización por fila usando el catálogo del producto
                def _canon_est_by_row(row):
                    raw = str(row.get("estatus", ""))
                    prod = str(row.get("producto", ""))
                    catalog = get_estatus_catalog_for_product(prod)
                    return canonicalize_from_catalog(raw, catalog, extra_synonyms=ESTATUS_SYNONYMS, min_ratio=0.90)

                try:
                    df_norm["estatus"] = df_norm.apply(_canon_est_by_row, axis=1)
                except Exception:
                    pass
            ...
            if nuevos_est:
                # Intentar asignar nuevos estatus al catálogo por producto cuando sea posible
                added_global = []
                for ne in nuevos_est:
                    ne_strip = ne.strip()
                    if not ne_strip:
                        continue
                    ...
                    if in_inb or in_mul or in_mej or (ne_strip in ESTATUS_OPCIONES):
                        continue

                    # Si vino columna producto, asignar al catálogo del producto más frecuente
                    target_tab = None
                    if "producto" in df_norm.columns:
                        rows_with = df_norm[df_norm["estatus"] == ne_strip]
                        if not rows_with.empty:
                            prod_mode = rows_with["producto"].mode()
                            prod_val = str(prod_mode[0]) if not prod_mode.empty else ""
                            p = (prod_val or "").strip().lower()
                            if "inbursa" in p:
                                target_tab = GSHEET_ESTATUSINBURSA_TAB
                            elif "multiva" in p:
                                target_tab = GSHEET_ESTATUSMULTIVA_TAB
                            elif "mejoravit" in p or "mejora" in p:
                                target_tab = GSHEET_ESTATUSMEJORAVIT_TAB

                    if target_tab:
                        try:
                            existing = load_catalog_from_gsheet(target_tab, [])
                    ...
                    # Siempre añadir globalmente para evitar bloqueos en import
                    ESTATUS_OPCIONES.append(ne_strip)
                    added_global.append(ne_strip)
